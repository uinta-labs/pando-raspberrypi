...
do_deploy:append() {
    # exclude from Pando OS the binaries with additional debug assertions (they
    # grow the used size in pando-boot and this potentially breaks hostapps
    # update)
    ...
    if echo ${MACHINEOVERRIDES} | grep -i "raspberrypi4-64"; then
        # exclude from raspberrypi4-64 based Pando OS builds the binaries which are for previous RaspberryPi versions
        ...
        # exclude RaspberryPi4 specific firmware from non raspberrypi4-64 based Pando OS builds
